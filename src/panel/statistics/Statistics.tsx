...
export const Statistics: React.FC<StatisticsProps> = ({
  ...
  if (show) {
    ...
    if (
      resolvedDrillDownLink && resolvedDrillDownLink.length > 0 &&
    ...
    const requests = (selectionStatistics.requests || 0) >= 0
      ? (
        <tr>
    ...
    const errors = (selectionStatistics.errors || 0) >= 0
      ? (
        <tr>
    ...
    let errorRate =
      (selectionStatistics.requests || 0) > 0 && (selectionStatistics.errors || 0) >= 0
        ? (
          <tr>
            ...
            <td className="table--td--selection">
              {roundPercentageToDecimal(
                2,
                ((100 / (selectionStatistics.requests || 0)) *
                  (selectionStatistics.errors || 0)).toString(),
              )}
            </td>
          </tr>
        )
    ...
    let avgResponseTime = (selectionStatistics.responseTime || 0) >= 0
      ? (
        <tr>
    ...
    let baseline = showBaselines && selectionStatistics.threshold
      ? (
        <tr>
          <td className="table--td--selection">
            Response Time Health (Upper Baseline)
          </td>
          {threshold}
        </tr>
      )
